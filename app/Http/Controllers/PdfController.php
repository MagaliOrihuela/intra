...
class PdfController extends Controller
{
    public function index(Request $request) 
    {
        ...
        $sqlPack = DB::select("SELECT dol.lot,dol.quantity,dol.standard,(dol.quantity/dol.standard) as tot,dol.package,a.article,u.unit,cu.short_name
                                    from d_ord_lots dol
                                    ...
                                    inner join c_clients cu on cu.user_id = dol.user
                                        where dor.order_id = $request->orderId and a.category_id = $request->catId
        ...
        foreach($packages as $pack){
            if($tmpPack <> $pack->package){
                ...
                $code = $client[0]->no_ped.$pack->package;
                $fpdf->code128(122, 30,$code, 50, 15, true);
                $fpdf->SetFont('arial','',10);
                $fpdf->RotatedText(141,63,$code,90);
